...
- 纯 Node CLI 前台持续采集闭环：`--attach <pid>` 可进入真实 attach、持续消费事件并打印 captured 摘要
...
- 当前范围已经收敛为：先把 request capture 做稳，不在这一轮同时做 response / stream capture
...
- [ ] 扩展到 response / stream capture
- [ ] 评估 Electron 目标的 attach 可达性与失败模式
- [ ] 为 response / stream capture 单独开 change
...
- `add-request-capture` 已经在当前分支打通第一版链路，路线图层面不再属于“下一步”
- 迭代 4 的 request 范围已经收敛，后续应该把 response / stream 作为独立能力推进
- 这样可以延续当前的产品切片思路：先看到它发了什么，再看到它收到了什么
