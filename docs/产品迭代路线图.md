...
- `prismtrace-host`：可运行 host、进程发现、readiness 判断和 attach 演示入口
...
- 本地 CLI 演示路径：`--discover`、`--readiness`、`--attach <pid>`
...
- `--attach <pid>` 本地演示入口
- attach / handshake failed / duplicate attach / detach 的确定性测试

当前仍未完成：

- 真实的 live instrumentation backend
- 真正的 probe 注入与 heartbeat
- 对外稳定的 detach / attach-status 命令面
...
1. 补完迭代 3：电动自行车 - 真实 live attach backend / probe bootstrap
2. 迭代 4：摩托车 - 单目标请求采集器
3. 迭代 5：汽车 - 本地可观测性控制台
4. 迭代 6：越野车 - 会话重建与可见性增强
5. 迭代 7：自动驾驶辅助 - 分析与解释层
...
- 它是“部分完成的电动自行车”迈向“真实可附着”的自然下一步
- 现在真正的技术分水岭已经不是 readiness，而是真实 live attach backend 和 probe bootstrap
- 它决定我们能否进入下一阶段的 request / response capture
