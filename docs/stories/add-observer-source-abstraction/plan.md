...
把 `PrismTrace` 从“单一 attach 工具”推进到“统一上层观测协议 + 多 source backend”的架构。

## 2. 已知前提

- 现有 `AttachProbeSource` 已经存在，只是还没被明确命名
- `Codex` 已进入 `CodexAppServerSource` 的实现起步阶段
...
- [ ] A.3 明确现有 attach 路线的新定位
  - 验证：attach 被视为 `AttachProbeSource`，不再代表整个 host 主入口
...
### 阶段 C：为 opencode 预留接入位

- [ ] C.1 新开 `OpencodeServerSource` 设计
  - 验证：明确从 `server + event/export` 入手，而不是 Bun attach

- [ ] C.2 固定第一版最小接入数据面
  - 验证：至少覆盖 `session / message / part / reasoning / tool / model / tokens / event`
...
先把这层架构明确下来，再继续各自推进 `Codex` 和 `opencode`。这样后续不会出现：
