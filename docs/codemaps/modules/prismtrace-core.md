...
承载 PrismTrace 的共享领域模型，避免 host 和后续 observer source 模块重复定义进程样本、归一化后的目标类型和 IPC message 协议。
...
- `IpcMessage::{to_json_line, from_json_line}`：序列化与解析本地 bridge / probe / capture 使用的 IPC 消息
...
- 当前已经清理 legacy attach 领域模型，不要把 attach session/readiness 概念重新塞回 core
