...
`local-console` 能力把 PrismTrace 从主要依赖 CLI 输出的观测原型，推进到一个可在浏览器中使用的本地可观测性控制台。它的重点不是一次性做完整分析，而是先把已经存在的 observer、request capture 和 session reconstruction 事实，稳定地呈现在一个统一的本地产品界面中。

本次能力面向“看见现在正在发生什么”和“快速回看最近捕获了什么”。因此规格优先约束：控制台入口、本地 source/activity 列表、基础 request 浏览和 source health 可见性；不要求在这一轮完成复杂 request inspector、response 深度解析或完整会话重建。

术语说明：

- 本地控制台：PrismTrace host 在本机暴露的 Web UI 与其最小 API surface。
- 活动时间线：按时间顺序展示的最近 observer、request 或 session 相关活动列表。
...
### Requirement: 控制台必须展示 source target 与其当前状态
本地控制台 MUST 展示可观测 source 列表，并为每个 source 暴露足够的状态信息，让用户判断该 source 当前是否可见、是否处于活跃观测状态，以及当前 source health 是否健康。

#### Scenario: 控制台显示 source 列表与 source 状态
- **WHEN** 用户打开本地控制台首页
- **THEN** 控制台展示本地 source 列表，并为每个 source 显示至少名称或显示名、runtime 类型以及当前 source 相关状态

#### Scenario: 没有活跃目标时仍保留空态解释
- **WHEN** 当前没有发现任何 target 或没有 active source
- **THEN** 控制台返回明确的空态说明，而不是只显示空白区域

#### Scenario: 带过滤条件时目标列表受过滤约束
- **WHEN** 用户以目标过滤条件启动本地控制台首页
- **THEN** 控制台展示的 target 列表只包含匹配过滤条件的目标，并保持与该范围一致的 source 状态视图

### Requirement: 控制台必须展示最近观测活动时间线
本地控制台 MUST 提供一个最近活动视图，用于按时间顺序呈现 observer、request capture 和 session reconstruction 等已经持久化或已知的观测活动，帮助用户快速理解“刚刚发生了什么”。

#### Scenario: 最近活动按时间顺序展示
- **WHEN** 系统中已经存在 observer、request 或 session 相关活动
...
### Requirement: 控制台必须暴露 source 健康与基础错误可见性
本地控制台 MUST 展示 active source 的健康摘要和基础错误可见性，使用户能够判断当前观测链路是否工作，以及失败发生在哪一层。

#### Scenario: 控制台显示 source 健康摘要
- **WHEN** 当前存在 active source 或最近 source 状态信息
- **THEN** 控制台展示 source 状态、能力摘要或最近错误提示中的至少一种可观察健康信息

#### Scenario: 控制台显示基础错误说明
- **WHEN** source、observer 或 request capture 路径出现已知失败
- **THEN** 控制台向用户展示基础错误说明或失败摘要，而不是要求用户仅从日志中排查

#### Scenario: 带过滤条件时健康信息反映过滤后的监控范围
- **WHEN** 用户以目标过滤条件启动本地控制台
- **THEN** 控制台展示的 observability health 摘要只反映匹配目标范围内的 source 与错误信息
