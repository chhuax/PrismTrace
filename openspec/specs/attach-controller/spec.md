# attach-controller Specification

## Purpose
定义 PrismTrace 在候选目标完成 readiness 判断之后，对单个目标发起 attach / detach 并维护 attach session 生命周期的能力边界，确保 host 能返回结构化的连接状态、失败分类和最小 probe bootstrap 结果，为后续 payload capture 奠定稳定的 attach 控制层。
