...
## Purpose
定义 PrismTrace 在 macOS 上发现候选 Node / Electron 进程的能力边界，确保 host 能返回结构化的进程目标集合，为后续 attach 和观测流程提供稳定输入。
## Requirements
...
每个被 discovery 返回的 process target MUST 包含 PrismTrace 后续 attach 工作需要的元数据：process id、可展示的应用名称、可执行路径和 runtime kind。
