# attach-readiness Specification

## Purpose
定义 PrismTrace 在候选进程发现结果之上输出 attach readiness 判断的能力边界，确保 host 能给出结构化的可附着性状态、原因说明和保守的不确定性表达，为后续 attach controller 与观测流程提供稳定输入。
