...
pub fn discover_latest_codex_socket(temp_dir: Option<&Path>) -> io::Result<Option<PathBuf>> {
    ...
    Ok(sockets.into_iter().map(|(path, _)| path).next())
}
...
mod tests {
    use super::{
        CodexObserverFactory, CodexObserverOptions, ObservedEventKind,
        capability_event_from_response, discover_latest_codex_socket, normalize_server_value,
        observed_event_kind_for_method,
    };
    use crate::observer::ObserverSourceFactory;
    use serde_json::json;
    use std::collections::{HashMap, VecDeque};
    use std::fs;
    use std::io;
