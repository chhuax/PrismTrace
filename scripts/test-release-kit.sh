...
for target in aarch64-apple-darwin x86_64-apple-darwin; do
  ...
  archive="$dist_dir/prismtrace-0.0.0-smoke-$target.tar.gz"
  extract_dir="$tmp_dir/extract-$target"
  mkdir -p "$extract_dir"
  tar -xzf "$archive" -C "$extract_dir"

  package_dir="$extract_dir/prismtrace-0.0.0-smoke-$target"
  ...
  install_prefix="$tmp_dir/prefix-$target"
